    ...
    <section>
        ...
        <form method="POST" enctype="multipart/form-data">
                        ...
                        if($size <= 1024 * 1024 * 5) {
                            if(in_array($type, ['jpg', 'jpeg', 'png', 'gif'])) {
                                if(move_uploaded_file($tmp, 'upload/' . $imgName)) {
                                    $sql = mysqli_query($conn, "UPDATE courses SET title = '$title', description = '$description', img = '$imgName', duration = '$duration' WHERE id = $id");
                                    if($sql) {
                                        echo '<div class="alert alert-success alert-dismissible fade show" role="alert">Course updated Sucessfully</div>';
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="input1" name="title" value="<?php echo $course['title'] ?>" aria-describedby="textHelp" required>
            </div>

            <div class="mb-3">
                <label for="exampleFormControlTextarea2" class="form-label">Title Description</label>
                <textarea class="form-control" id="exampleFormControlTextarea2" name="description" value = "<?php echo $course['description'] ?>" rows="5"  required></textarea>
            </div>

            <div class="mb-3">
              <label for="input1" class="form-label">Chooses a image for course</label>
              <input class="form-control" type="file" name="img" value="<?php echo $course['img']; ?>" id="input1" multiple required>
            </div>

            <div class="form-floating">
                <select class="form-select" id="floatingSelect" name="duration" value="<?php echo $course['duration'] ?>" aria-label="Floating label select example"  required>
